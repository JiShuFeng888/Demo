...
function initCategoryDate(){
  let oMainIn=document.querySelector(".main-in");
  let obj2=initCategoryJsonData();
  var html2 = template('demo', obj2);
...
function  initTextRow(){
    let oDes=document.querySelectorAll(".mask-info-des");
...
function initNav(){
      ...
      print("????????????????");
        window.onscroll=function(){
            let offsetY=getPageScroll().y;
            if(offsetY>=oNav.offsetHeight){
                oNav2.style.top=0+"px";
            }else{
                oNav2.style.top=-100+"px";
            }
        };
}
...
function waterfall(){
    ...
    let mainInWidth=oMain.offsetWidth;
    let oItemsWidth=oItems[0].offsetWidth;
    let cols=Math.floor(mainInWidth/oItemsWidth);
    ...
    for(let i=0;i<oItems.length;i++){
        ...
        if(i<cols){
            ...
            let oShortBox=oItems[minIndex];
            let oShortBoxLeft=oShortBox.offsetLeft;
            item.style.position = "absolute";
            item.style.left=oShortBoxLeft+"px";
            item.style.top=minHeight+20+"px";
            rowHeight[minIndex]+=item.offsetHeight+20;
        }
    }
    let maxHeight=Math.max.apply(this,rowHeight);
    oMain.style.height=maxHeight+30+"px";
}
